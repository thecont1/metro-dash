...
pub async fn bind() -> TcpListener {
    for port in PORT_START..=PORT_START.saturating_add(PORT_RANGE) {
        if let Ok(listener) = reuseable_listener(("127.0.0.1", port)) {
            return listener;
        }
    }
    panic!(
...
/// Bind with `SO_REUSEADDR` (and `SO_REUSEPORT` on macOS) so that restarting
/// the dev server in quick succession doesn't fail with `EADDRINUSE`.
fn reuseable_listener(addr: impl std::net::ToSocketAddrs) -> std::io::Result<TcpListener> {
    let std_addr = addr
        .to_socket_addrs()?
        .next()
